...
# Retrieve unique chief mentor IDs
    ...
    for student_id, mentor_id in assigned_mentors.items():
        ...
        student_ids.append(student_id) # List of student IDs
    
    base_url = "http://uat.traditionalyoga.co.in/yogaAdmin/student/autoMapping/sendMails"
    query_params = f"studentList={','.join(map(str, student_ids))}&courseId={course_id}"
    ...
    try:
        response = requests.post(final_url,headers=headers)  # Use json parameter to send data as JSON
        if response.status_code == 200:
            print("API call to Java API was successful")
        else:
            print(f"API call to Java API failed with status code: {response.status_code}")
            print(f"Response content: {response.text}")
    except Exception as e:
        print(f"Error while making the API call: {str(e)}")
